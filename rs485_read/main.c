/* 
  C sample application that reads from the raspicomm's RS-485 Port  
*/
...
int main(int argc, char** argv)
{
  ...
  /* read in a loop */
  printf("start reading from the rs-485 port a maximum of %i bytes\n", READ_BUFFER_SIZE);

  i = 0;

  while (i < READ_BUFFER_SIZE)      /* read until the readbuffer is full */
  {
    readBytes = read(fd, &data, 1); /* try to read only 1 byte at once */
    if (readBytes > 0)              /* if we received 1 byte */
      readBuffer[i++] = data;       /* store it in the read buffer */    
  }

  /* print the received bytes */
  printf("we received the following bytes:\n");
  for (i = 0; i < READ_BUFFER_SIZE; i++) {
    printf("[%i]: 0x%2X", i, readBuffer[i]);
